...
class TestDeepLearningIntegration:
    ...
    def test_neural_curve_training_and_prediction(self):
        ...
        # Random inputs
        X_train = np.random.rand(num_samples, num_points, settings.input_features).astype(
            np.float32
        )
        ...
        # 3. Train Ensemble
        histories = predictor.train_ensemble(X_train=X_train, y_train=y_train)
        ...
        result = predictor.predict(input_values=input_values, return_uncertainty=True)
