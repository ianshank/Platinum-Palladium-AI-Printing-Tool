...
class PrintingSimulator:
    ...
    def _generate_target_curve(self) -> np.ndarray:
        ...
        # Linear ramp from 0.1 (paper base) to 2.0 (Dmax)
        return np.linspace(0.1, 2.0, num_steps)
    ...
    def _predict_with_physics(self, state: PrintState) -> np.ndarray:
        ...
        base_gamma = 1.6 + 0.4 * state.metal_ratio

        # Contrast agent effect on gamma
        if state.contrast_active > 0.5:
            gamma = base_gamma + 0.1 * state.contrast_amount
        else:
            gamma = base_gamma
        ...
        dmin = 0.08 + 0.02 * (state.humidity / 100.0)

        # Maximum density (Dmax) - affected by exposure and chemistry
        base_dmax = 1.5 + 0.5 * state.metal_ratio  # Pt gives higher Dmax
        exposure_factor = 1.0 - np.exp(-state.exposure_time / 120.0)  # Saturation curve
        dmax = dmin + (base_dmax - dmin) * exposure_factor

        # Apply humidity effect on Dmax
        humidity_factor = 1.0 + 0.1 * (state.humidity - 50.0) / 50.0
        dmax = dmax * np.clip(humidity_factor, 0.9, 1.1)

        # Generate characteristic curve
        # Using a modified power function with shoulder and toe
        shoulder_position = 0.85
        toe_position = 0.15
        ...
        shoulder_compression = 0.3 * (input_values - shoulder_position) ** 2
        response[shoulder_mask] -= shoulder_compression[shoulder_mask]

        # Apply toe expansion
        toe_mask = input_values < toe_position
        toe_expansion = 0.2 * (toe_position - input_values) ** 2
        ...
        noise = rng.normal(0, 0.01, num_steps)
        densities += noise
    ...
    def calculate_linearity_score(
        ...
        # Using exponential decay with characteristic scale of 0.5
        linearity_score = np.exp(-rmse / 0.5)
    ...
    def calculate_reward(self, state: PrintState) -> float:
        ...
        monotonicity = num_increasing / (len(diffs) + 1e-6)

        # Smoothness score (penalize large second derivatives)
        second_diff = np.diff(diffs)
        smoothness = 1.0 / (1.0 + np.std(second_diff) * 10)
    ...
    def get_initial_state(
        ...
        Args:
            metal_ratio: Initial metal ratio (0=Pd, 1=Pt)
        ...
        state = PrintState(
            ...
            densities=np.zeros(self.state_config.num_density_steps, dtype=np.float32),
        )
