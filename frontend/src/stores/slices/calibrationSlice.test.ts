...
describe('calibrationSlice', () => {
  ...
  describe('Calibration Lifecycle', () => {
    ...
    it('setMeasurements stores measurement data', () => {
      ...
      expect(store.getState().calibration.current?.measurements).toEqual(
        measurements
      );
    });
    ...
    it('addMeasurement appends to existing measurements', () => {
      ...
      expect(store.getState().calibration.current?.measurements).toHaveLength(
        1
      );
    });
    ...
    it('updateMeasurement modifies a specific step', () => {
      ...
      expect(
        store.getState().calibration.current?.measurements[0]?.measuredDensity
      ).toBe(0.15);
    });
  });
  ...
  describe('Calibration History', () => {
    it('saveCalibration adds current to history', () => {
      ...
      store
        .getState()
        .calibration.saveCalibration('My Calibration', 'Test notes');

      expect(store.getState().calibration.history).toHaveLength(1);
      expect(store.getState().calibration.history[0]?.name).toBe(
        'My Calibration'
      );
    });
