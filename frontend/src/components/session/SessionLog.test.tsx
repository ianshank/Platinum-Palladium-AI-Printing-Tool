...
vi.mock('@/stores', () => ({
    useStore: (selector: (s: any) => any) => selector({ session: mockSessionState }),
}));
...
describe('SessionLog', () => {
    ...
    describe('With Records', () => {
        ...
        it('displays most used paper stat', () => {
            render(<SessionLog />);
            expect(screen.getByTestId('stat-paper')).toHaveTextContent('Arches Platine');
        });
        ...
        it('shows paper type in row', () => {
            render(<SessionLog />);
            expect(screen.getByTestId('record-rec-1')).toHaveTextContent('Arches Platine');
        });
