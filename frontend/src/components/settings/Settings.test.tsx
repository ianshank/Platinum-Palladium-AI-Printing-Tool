...
describe('Settings', () => {
    ...
    describe('Theme', () => {
        ...
        it('dark button is pressed when theme is dark', () => {
            render(<Settings />);
            expect(screen.getByTestId('theme-dark-btn')).toHaveAttribute('aria-pressed', 'true');
            expect(screen.getByTestId('theme-light-btn')).toHaveAttribute('aria-pressed', 'false');
        });
    ...
    describe('Sidebar', () => {
        ...
        it('sidebar toggle has switch role', () => {
            render(<Settings />);
            expect(screen.getByTestId('sidebar-toggle')).toHaveAttribute('role', 'switch');
        });

        it('sidebar toggle reflects current state', () => {
            render(<Settings />);
            expect(screen.getByTestId('sidebar-toggle')).toHaveAttribute('aria-checked', 'true');
        });
    ...
    describe('Application Info', () => {
        it('shows API URL', () => {
            render(<Settings />);
            expect(screen.getByTestId('info-api-url')).toHaveTextContent('http://localhost:8000');
        });
