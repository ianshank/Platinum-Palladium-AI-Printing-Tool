...
export function Step1Upload() {
  ...
  // Store
  const startCalibration = useStore(
    (state) => state.calibration.startCalibration
  );
  const setMeasurements = useStore(
    (state) => state.calibration.setMeasurements
  );
    ...
    uploadScan(
      { file: uploadedFile, tabletType },
      {
        onSuccess: (data: ScanUploadResponse) => {
          ...
          const stepCount =
            tabletConfig.stepTablets.find((t) => t.id === tabletType)?.steps ??
            21;
          const wizardTabletType = `${stepCount}-step` as
            | '21-step'
            | '31-step'
            | '41-step'
            | 'custom';
  ...
  return (
    <Container>
      ...
      <div>
        ...
        <TabletGrid>
          {tabletConfig.stepTablets.map((type) => (
            <TabletOption key={type.id} $selected={tabletType === type.id}>
              <input
                type="radio"
                name="tabletType"
                value={type.id}
      ...
      {(error || uploadError) && (
        <ErrorMessage>{error || (uploadError as Error)?.message}</ErrorMessage>
      )}
