...
export function Step2Analyze() {
  ...
  const analysisResult = useMemo(() => {
    if (!currentCalibration || !currentCalibration.measurements.length)
      return null;

    const densities = currentCalibration.measurements.map(
      (m) => m.measuredDensity
    );
    ...
    <Container>
      ...
      >
        ...
        {qualityAssessment && (
          <QualityBadge
            $type={
              qualityAssessment.overall === 'excellent' ||
          ...
          >
            Quality: {qualityAssessment.overall.toUpperCase()} (
