...
const mockStatistics = {
  ...
  date_range: ['2025-06-15T10:30:00', '2026-02-01T14:20:00'] as [
    string,
    string,
  ],
...
describe('Dashboard', () => {
  ...
  describe('Error State', () => {
    ...
    it('shows retry button on error', async () => {
      (api.statistics.get as Mock).mockRejectedValue(new Error('Server down'));
      (api.calibrations.list as Mock).mockRejectedValue(
  ...
  describe('Success State — Statistics Cards', () => {
    ...
    it('shows total calibrations card', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId('stat-total-calibrations')
        ).toBeInTheDocument();
      });
  ...
  describe('Success State — Recent Calibrations Table', () => {
    ...
    it('renders all calibration rows', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId('calibration-row-cal-001')
        ).toBeInTheDocument();
        expect(
          screen.getByTestId('calibration-row-cal-002')
        ).toBeInTheDocument();
        expect(
          screen.getByTestId('calibration-row-cal-003')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Empty State', () => {
    ...
    it('shows "Start Calibration" CTA button', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId('start-first-calibration')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Quick Actions', () => {
    ...
    it('renders all 4 quick action cards', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId('action-new-calibration')
        ).toBeInTheDocument();
