...
export const mctsApi = {
   ...
   */
  evaluate: async (request: MCTSEvaluateRequest): Promise<MCTSEvaluateResponse> => {
    const response = await apiClient.post<{
  ...
  getStatus: async (): Promise<MCTSStatusResponse> => {
    const response = await apiClient.get<{
      ...
      parameter_ranges: Record<string, { min: number; max: number; default: number; unit: string }>;
    }>('/api/mcts/status');
  ...
  getRecommendations: async (
    ...
    const response = await apiClient.get<MCTSRecommendationsResponse>('/api/mcts/recommendations', {
      params: {
   ...
   */
  startTraining: async (request: MCTSTrainRequest = {}): Promise<MCTSTrainResponse> => {
    const response = await apiClient.post<{
      session_id: string;
      status: string;
      message: string;
   ...
   */
  submitFeedback: async (feedback: MCTSFeedbackRequest): Promise<{ success: boolean; message: string }> => {
